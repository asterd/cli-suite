...
/// Why an agent stream was truncated.
...
/// Result summary from an agent writer.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct AgentWriteSummary {
...
/// Writes minified NDJSON records and enforces output limits.
#[derive(Debug)]
pub struct AgentNdjsonWriter<'a, W: Write + ?Sized> {
    inner: &'a mut W,
    limits: OutputLimits,
    records: usize,
    bytes: usize,
...
impl<'a, W: Write + ?Sized> AgentNdjsonWriter<'a, W> {
    /// Create an agent NDJSON writer.
    ...
    /// agent output can preserve the required summary-first contract.
    ...
    pub fn finish(self, warn_schema: &str) -> Result<AgentWriteSummary> {
        let summary = AgentWriteSummary {
...
mod tests {
    ...
    #[test]
    fn agent_writer_minifies_records() -> Result<()> {
        let mut output = Vec::new();
        let mut writer = AgentNdjsonWriter::new(&mut output, OutputLimits::default());
        assert!(writer.write_record(&json!({ "s": "test.v1", "t": "summary" }))?);
    ...
    #[test]
    fn agent_writer_truncates_by_record_limit() -> Result<()> {
        let mut output = Vec::new();
        ...
        let mut writer = AgentNdjsonWriter::new(&mut output, limits);
        assert!(writer.write_record(&json!({ "s": "test.v1", "t": "summary" }))?);
        assert!(!writer.write_record(&json!({ "s": "test.v1", "t": "detail" }))?);
        let summary = writer.finish("test.warn.v1")?;
        assert_eq!(summary.truncated, Some(TruncationReason::MaxRecords));
        assert_eq!(
            String::from_utf8_lossy(&output),
            "{\"s\":\"test.v1\",\"t\":\"summary\"}\n{\"s\":\"test.warn.v1\",\"t\":\"warn\",\"code\":\"truncated\",\"reason\":\"max_records\",\"trunc\":true}\n"
        );
        Ok(())
    }

    #[test]
    fn agent_writer_truncates_by_byte_limit() -> Result<()> {
        let mut output = Vec::new();
        let limits = OutputLimits {
            max_records: 10,
            max_bytes: 3,
        ...
        assert_eq!(
            String::from_utf8_lossy(&output),
            "{\"s\":\"test.v1\",\"t\":\"summary\"}\n{\"s\":\"test.warn.v1\",\"t\":\"warn\",\"code\":\"truncated\",\"reason\":\"max_bytes\",\"trunc\":true}\n"
        );
        Ok(())
    }

    #[test]
    fn agent_writer_strict_truncation_errors_after_warning() -> Result<()> {
        let mut output = Vec::new();
        let limits = OutputLimits {
            max_records: 0,
            max_bytes: 1_000,
